...
$cu = current_user($pdo);
if ($cu && user_can_access_admin($cu)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $st->execute([$email]);
    $u = $st->fetch();
    if ($u && password_verify($pass, $u['password_hash']) && user_can_access_admin($u)) {
        login_user((int) $u['id']);
        header('Location: index.php');
        exit;
    }
    ...
    <section class="app-page">
        <div class="container">
            <div class="app-card" style="max-width: 420px; margin: 0 auto;">
                <h1 class="section-title" style="text-align: center;">Админ-панель</h1>
